...
im=imread('CLab2/Task1/Harris_1.jpg'); % Step 0: Read Image
figure ;imshow(im);
im1=rgb2gray(im);
figure ;imshow(im1);
...
dy = dx';
Ix = imfilter(im1, dx);    
% Step 1: Compute the image derivatives Ix and Iy
Iy = imfilter(im1, dy);
g = fspecial('gaussian',9,2); 
% Step 2: Generate Gaussian filter 'g' of size 9x9 and standard deviation Sigma=2.
Ix2 = imfilter(Ix.^2, g); 
% Step 3: Smooth the squared image derivatives to obtain Ix2, Iy2 and IxIy
figure;imshow(Ix2);
Iy2 = imfilter(Iy.^2, g);
figure;imshow(Iy2);
IxIy = imfilter(Ix.*Iy, g);
figure;imshow(IxIy); 
%Display the images obtained in different steps
...
for i=2:1:r-1 
    for j=2:1:c-1
     ...
     %(1) Build autocorrelation matrix for every singe pixel considering a window of size 3x3
     E(i,j)=min(eig(M)); 
     %(2)Compute Eigen value of the autocorrelation matrix and save the minimum eigenvalue as the desired value.
    end
end
t=toc;
...
[a, b] = ind2sub([r c],RIX);
%The mapping from linear indexes to subscript equivalents for the matrix
figure; imshow(im1, []); hold on; xlabel('Max 81 points');
% Get the coordinates with maximum cornerness responses     
for i=1:81, 
	plot(a(i), b(i), 'r+'); 
end  
