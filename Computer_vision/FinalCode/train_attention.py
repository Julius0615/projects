...
parser.add_argument('--model_dir', default='experiments/attention', help="Directory containing params.json")
parser.add_argument('--restore_file', default=None,
                    help="Optional, name of the file in --model_dir containing weights to reload before \
                    training")  # 'best' or 'train'
parser.add_argument("--model", default="FERAttentionNet")


def evaluate(model, loss_fn, dataloader, metrics, params,writer=None,epoch=0,model_dir=""):
    ...
    for batch in dataloader:
        data_batch, labels_batch, landmark_mask = batch['image'],batch['labels'],batch['landmark_mask']
        ...
        y, att, g_att, g_ft = model(data_batch)
        loss = loss_fn(y, labels_batch) + mse(att, data_batch * landmark_mask)
        # extract data from torch Variable, move to cpu, convert to numpy arrays
        output_batch = y.data.cpu().numpy()
    ...
    writer.add_images(tag="Eval/{}_image".format(count), img_tensor=torch.stack(
        (data_batch[0], g_ft[0][:3], att[0], (data_batch[0] * landmark_mask[0]))).detach().cpu(),
                      global_step=epoch, dataformats="NCHW")
    tu.save_image(tu.make_grid(torch.stack(
        (data_batch[0], g_ft[0][:3], att[0], (data_batch[0] * landmark_mask[0]))).detach().cpu(),normalize=True,scale_each=True),
                  os.path.join(model_dir,"image_{}.png".format(epoch)))
...
def train(model, optimizer, loss_fn, dataloader, metrics, params):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, batch in enumerate(dataloader):
            train_batch, labels_batch,landmark_mask = batch['image'],batch['labels'],batch['landmark_mask']
            train_batch = torch.Tensor(train_batch)
            labels_batch = torch.LongTensor(labels_batch)
            landmark_mask = torch.FloatTensor(landmark_mask)
            # move to GPU if available
            if params.cuda:
            ...
            y, att, g_att, g_ft = model(train_batch)
            attention_loss = mse(att,train_batch*landmark_mask)
            attention_loss = 10*torch.clamp(attention_loss,max=30)

            loss = loss_fn(y, labels_batch)+attention_loss
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, loss_fn, metrics, params, model_dir,
    ...
    for epoch in range(params.num_epochs):
        ...
        val_metrics = evaluate(model, loss_fn, val_dataloader, metrics, params,summarywriter,epoch,model_dir)
...
if __name__ == '__main__':
    ...
    # fetch dataloaders
    dataloaders = data_loader.fetch_aug_dataloader(['train', "val"], datas, params)
